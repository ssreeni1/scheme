...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        ...
        holder = scheme_eval(expressions.first, env, True)
        return holder
    else:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5, 
        bounded_val = scheme_eval(expressions.rest.first, env)
        env.define(target, bounded_val)
        return target
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        defined_func = target.first
        bounded_lam = LambdaProcedure(expressions.first.rest, expressions.rest, env) 
        env.define(defined_func, bounded_lam)   
        return defined_func
        # END PROBLEM 9
...
def do_and_form(expressions, env):
    ...
    placeholder = scheme_eval(expressions.first, env)
    if scheme_falsep(placeholder):
        return False
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        formal_list = Pair(bindings.first.first, formal_list)
        arg_list = Pair(scheme_eval(bindings.first.rest.first, env), arg_list)
        check_formals(formal_list)
        bindings = bindings.rest
    return env.make_child_frame(formal_list, arg_list)
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 20
    check_form(expressions, 2)
    syms = expressions.first
    check_formals(syms)
    check_form(syms, 0)
    named = syms.first
    params = syms.rest
    func_body = expressions.rest
    env.define(named, MacroProcedure(params, func_body, env))
    return named
    # END Problem 20
